  ...
  new LocalStrategy(
    {
      ...
      ).lean(); // lean(): chuyển đổi sang đối tượng js thuần

      if (!docUser || !docUser.id) {
        return done(null, false, { message: 'Email không tồn tại' });         // sai email
      }
      else {
        require('bcryptjs').compare(password, docUser.password, (err, data) => {
          if (err) {
            ...
            return done(null, false, { message: 'Sai mật khẩu' }); // sai mật khẩu
          }
          
          return done(null, docUser);                                    // Người dùng hợp lệ
        });
      }
    }
  )
